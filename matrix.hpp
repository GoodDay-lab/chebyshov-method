...
public:
  ...
  friend void mulvt(const Matrix &in_mat, const Vector &in_vec,
                    Vector &out_vec);
...
public:
  ...
  auto operator()(iT i, iT j) const -> dataType {
    return dataPtr[i * _colSize + j];
  };
  auto operator()(iT i, iT j) -> dataType & {
    return dataPtr[i * _colSize + j];
  };
